...
def RNN(Inputs, SeqLens, Scope):
	...
	with tf.compat.v1.variable_scope(Scope):
		...
		maxLen = tf.reduce_max(input_tensor=SeqLens, axis=0)
		...
		[fw_out, bw_out], _ = tf.compat.v1.nn.bidirectional_dynamic_rnn(cell_fw=forward, cell_bw=backward, inputs=Inputs, time_major=True, dtype=tf.float32,sequence_length=tf.cast(SeqLens, tf.int64))
