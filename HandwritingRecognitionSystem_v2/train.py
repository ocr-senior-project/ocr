...
def run(num_training_imgs, list_file, imgs_path, labels_path):
	...
	values = tf.compat.v1.placeholder(dtype=tf.int32, shape=[None])
	shape = tf.compat.v1.placeholder(dtype=tf.int64,shape=[2])

	# Make targets
	targets = tf.SparseTensor(indices, values, shape)

	# Compute Loss
	losses = tf.compat.v1.nn.ctc_loss(targets, logits, SeqLens)

	loss = tf.reduce_mean(input_tensor=losses)

	TrainLoss_s = tf.compat.v1.summary.scalar('TrainLoss', loss)
	...
	grad, _ = tf.clip_by_global_norm(tf.gradients(ys=loss, xs=tvars), cfg.MaxGradientNorm)

	optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate=cfg.LearningRate)
	...
	OverallTrainingLoss_s = tf.compat.v1.summary.scalar('OverallTrainingLoss', OverallTrainingLoss)
	OverallTrainingError_s = tf.compat.v1.summary.scalar('OverallTrainingError', OverallTrainingError)
	OverallValidationLoss_s = tf.compat.v1.summary.scalar('OverallValidationLoss', OverallValidationLoss)
	OverallValidationError_s = tf.compat.v1.summary.scalar('OverallValidationError', OverallValidationError)
	...
	LocalTrainSummary = tf.compat.v1.summary.merge([TrainLoss_s, TrainError_s])

	OverallSummary = tf.compat.v1.summary.merge([OverallTrainingLoss_s, OverallTrainingError_s, OverallValidationLoss_s, OverallValidationError_s])

	SummaryWriter = tf.compat.v1.summary.FileWriter(cfg.LogDir, session.graph)
	...
	try:
		for epoch in range(cfg.StartingEpoch, cfg.NEpochs):
			...
			session.run(tf.compat.v1.assign(phase_train, True))
